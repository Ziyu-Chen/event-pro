...
class Register extends React.Component<RegisterProps, User & RegisterState>{
  ...
  handleSubmit = (
    ...
    .catch((err) => {
      if (err.response.status === 409) {
        this.setState({failed: true, error: `The email address ${this.state.email} is already in use.`})
      } else {
        this.setState({failed: true, error: "There seem to be some issues."})
  ...
  render() {
    ...
    <div className="register">
      ...
      <ValidatorForm 
        onSubmit={() => {
          this.handleSubmit( this.state.email, this.state.password, this.state.name, this.state.description, this.state.website, this.state.logoUrl)
        }
      }
      >
        <FormControl>
          ...
          <TextValidator 
            ...
            errorMessages={["this field is required", "your passwords don't match"]}
      ...
      <Dialog open={this.state.succeeded}>
        ...
        <DialogContent>
          <DialogContentText>You have successfully created an account. Now you can log in and promote your events! </DialogContentText>
        </DialogContent>
        <DialogActions>
